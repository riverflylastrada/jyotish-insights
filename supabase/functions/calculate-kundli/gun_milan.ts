/**
 * 36-point Ashtakoota (Guna Milan) compatibility engine.
 *
 * Computes the 8 North-Indian Kootas (Varna, Vasya, Tara, Yoni, Graha Maitri,
 * Gana, Bhakoot, Nadi) from the Moon nakshatra + rashi of two partners.
 * Pure O(1) math — no ephemeris calls.
 *
 * Logic extracted verbatim from `src/pages/app/Compatibility.tsx` (the
 * existing frontend-only implementation) so both server and client share
 * identical rules with zero drift.
 *
 * Parity oracle: AstroSage (Lahiri ayanamsa, standard 36-point Ashtakoota)
 * and classical reference (Muhurta Chintamani).
 */
...
/** Rashi lords: 1-based sign → planet key */
...
/**
 * Vasya type by rashi: 1=Chatushpada, 2=Manushya, 3=Jalachara, 4=Vanachara, 5=Keeta
 */
function vasyaType(rashi: number): { name: string; type: number } {
  ...
  return { name: "Manushya", type: 2 }; // 3, 6, 7, 11
}

/** Bride (row) × Groom (col) → Vasya points */
...
/** Normalize a nakshatra name to lowercase, letters only (for lookup keys). */
export function normalizeName(name: string): string {
  return name.toLowerCase().replace(/[^a-z]/g, "");
}

/** Get 1-based nakshatra index from name. */
...
function computeVarna(groomRashi: number, brideRashi: number) {
  const score = (r: number) => {
    if ([4, 8, 12].includes(r)) return 4; // Brahmin
    if ([1, 5, 9].includes(r))  return 3; // Kshatriya
    if ([2, 6, 10].includes(r)) return 2; // Vaishya
    return 1;                              // Shudra
  };
...
function computeGana(groomNorm: string, brideNorm: string) {
  ...
  else points = 0; // Manushya + Rakshasa
...
/**
 * Compute 36-point Ashtakoota (Guna Milan) from groom/bride Moon data.
 *
 * @param groomNakshatra  Nakshatra name (e.g. "Purva Phalguni")
 * @param groomRashi      1-based rashi / sign number (1=Mesha … 12=Meena)
 * @param brideNakshatra  Nakshatra name
 * @param brideRashi      1-based rashi / sign number
 */
