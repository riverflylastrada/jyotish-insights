/**
 * South-Indian 10-Porutham marriage compatibility engine.
 *
...
/** Count nakshatras from `from` to `to` (1-based, wrapping at 27). */
const countStars = (from: number, to: number): number =>
  ((27 + (to - from)) % 27) + 1;

/** Count rasis from `from` to `to` (1-based, wrapping at 12). */
const countRasis = (from: number, to: number): number =>
  ((12 + (to - from)) % 12) + 1;

/** Derive 1-based rasi number from nakshatra (1–27) and pada (1–4). */
export function rasiFromNakshatraPada(nak: number, pada: number): number {
  const nakDur = 360 / 27;
...
function dinaPoruthamSouth(
  ...
  const count = countStars(boyNak, girlNak); // count from boy's star
...
function mahendraPorutham(boyNak: number, girlNak: number): boolean {
  const rem = countStars(girlNak, boyNak);
  return MAHENDRA_ARRAY.includes(rem);
}
...
function streeDheergaPorutham(boyNak: number, girlNak: number): boolean {
  const count = countStars(girlNak, boyNak);
  return count > STREE_DHEERGA_THRESHOLD_SOUTH;
}

// ─── Yoni (South) ──────────────────────────────────────────────────────────

/** Maps nakshatra index (0-based) to yoni animal index (0-based). */
...
/** South-Indian yoni enemy pairs (ordered tuples). */
...
const FOOT_AVAROGA = [9, 18, 27];
const WAIST_AAROGA = [2, 11, 20];
const WAIST_AVAROGA = [8, 17, 26];
const STOMACH_AAROGA = [3, 12, 21];
const STOMACH_AVAROGA = [7, 16, 25];
// PyJHora bug: neck_aaroga_rajju = [413, 22] (should be [4, 13, 22]).
// We replicate the bug so our results match the PyJHora CSV oracle.
const NECK_AAROGA = [413, 22];
const NECK_AVAROGA = [6, 15, 24];
...
/** Vasya rasi list maps each rasi (0-indexed) to a set of "vasya" rasis. */
...
/** Nadi category: 0 = Adi (Vata), 1 = Madhya (Pitta), 2 = Antya (Kapha). */
...
/**
 * Compute 10 South-Indian poruthams from boy/girl Moon nakshatra + pada.
 * O(1) pure math — no ephemeris lookups.
 */
